...
export default function DeleteList() {
  ...
  const fetchLogs = async (page) => {
    try {
      const response = await api.get("/admin/recipes/logs/deleted", {
        params: { page: page, size: logsPerPage },
      });
      setLogs(response.data.content);
      setTotalPages(response.data.totalPages);
    } catch (error) {
      console.error("삭제된 레시피 로그 불러오기 실패:", error);
    }
  };
  ...
  const openModal = (log) => {
    setViewReason(log.reason || "사유 없음");
    setModalOpen(true);
  };
      ...
      <div className="white-box">
        ...
        <div className="top-bar">
          ...
          <div className="search-box">
            <input type="text" placeholder="레시피 검색 (미구현)" />
            ...
            {logs.map((log, index) => (
              <tr key={log.targetId}>
                ...
                <td className="buttons">
                  ...
                  <button
                    className="detailSee"
                    onClick={() => navigate(`/recipes/${log.targetId}`)}
                  >
                    상세보기
                  </button>
                </td>
