...
export default function DeleteCommentList() {
  ...
  const fetchLogs = async (page) => {
    try {
      const response = await api.get("/admin/comments/logs/deleted", {
        params: { page: page, size: logsPerPage },
      });
      setLogs(response.data.content);
      setTotalPages(response.data.totalPages);
    } catch (error) {
      console.error("삭제된 댓글 로그 불러오기 실패:", error);
    }
  };
            ...
            {logs.map((log, index) => (
              <tr key={log.targetId}>
                ...
                <td className="buttons">
                  <button className="detailSee" onClick={() => navigate(`/posts/${log.targetId}`)}>상세보기</button>
                </td>
              </tr>
            ))}
          </tbody>
